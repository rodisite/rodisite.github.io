...
function display_transaction(obj) {
    ...
    let new_div = document.createElement('div');        // Добавление элемента div: сумма
    new_div.className = 'transaction_summ';             //
    let text_div = document.createTextNode(obj.sum)     //
    ...
    text_div = document.createTextNode('x');
    new_div.appendChild(text_div);
    element_list.appendChild(new_div);
...
function display_statistics(obj) {
    ...
    if (all_sum[i][j] == 0) {                               // Если раннее трат по данной подкатегории не было,
        ...
        new_div.id = id_div;
        text_div = document.createTextNode(`${obj.sum}`);
        new_div.appendChild(text_div);
        element_list.appendChild(new_div);
        ...
        stat_div.textContent = +stat_div.textContent + obj.sum;
    }
}
...
window.onload = select_category(), show_transactions(), show_statistics(), remove();
